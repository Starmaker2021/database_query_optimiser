...
//Optimizer optimization detailed steps:
    //1. According to the standard structure, decompose layer by layer until the bottom product (AxBxC)
        //1.1. Create a projectDownList to assist in the moving down of the project
            //1.1.1.projectDownList records the properties that the lower layer needs to project to the upper layer
    ...
    //3. Generate a new Join with the smallest cost in turn, and generate a left deep tree
        ...
        //3.3.When the parentNode is paired with other TreeNodes, it will be used as a left node
        //3.4.Loop generation until the pairs are empty, generate a left deep tree with most restrictive relations first
        //3.5.Finally, perform a projectDown again, project the required attributes, and the optimization is completed
    ...
    // and the number of occurrences during the top-down processing of a specification query
